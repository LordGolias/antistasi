...
if (([_unit] call isMember) && ((server getVariable "enableMemAcc"))) exitWith {
	// XLA fixed arsenal
	if (hayXLA) then {
		["Open",[nil,_box,_unit]] call XLA_fnc_arsenal;
	} else {
...
if ( _backpack != "" ) then
	...
	if (not (_backpack in unlockedBackpacks)) then
		...
		if !( _inScope ) then
			...
			// XLA fixed arsenal
			if (hayXLA) then {
				missionNamespace setVariable [ "XLA_fnc_arsenal_data", nil ];
				["Preload"] call XLA_fnc_arsenal;
				_tmpWhitelistBackpacks = ( missionNamespace getVariable "XLA_fnc_arsenal_data" ) select 5;
			} else {
...
// XLA fixed arsenal
if (hayXLA) then {
	["Open",[nil,_box,_unit,false]] call XLA_fnc_arsenal;
} else {
