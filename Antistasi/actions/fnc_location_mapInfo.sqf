...
hint format ["Total pop: %1\nFIA Support: %2\nAAF SUpport: %3 \n\nDestroyed Cities: %4\n\nClick on the location",
	_pop, _popFIA, _popAAF, {_x in AS_P("destroyedLocations")} count (call AS_location_fnc_cities)];
...
while {visibleMap} do {
	if (count posicionTel > 0) then {
		...
		if (_type == "airfield") then {
			if (_side == "AAF") then {
				_texto = format ["AAF %1", _location call AS_fnc_location_name];
				...
				_texto = format ["FIA %1\n%2", _location call AS_fnc_location_name, [_location] call AS_fnc_getGarrisonAsText];
			};
		};
		if (_type == "base") then {
			if (_side == "AAF") then {
		...
		if (_type == "powerplant") then {
			if (_side == "AAF") then {
				...
				_texto = format ["FIA Powerplant%1",[_location] call AS_fnc_getGarrisonAsText];
			};
			if (_location in AS_P("destroyedLocations")) then {_texto = format ["%1\nDESTROYED",_texto]};
		};
		...
		if (_type in ["outpost", "outpostAA"]) then {
			if (_side == "AAF") then {
				...
				_texto = format ["FIA Outpost%1",[_location] call AS_fnc_getGarrisonAsText];
			};
		};
		if (_type == "seaport") then {
			if (_side == "AAF") then {
				...
				_texto = format ["FIA Seaport%1",[_location] call AS_fnc_getGarrisonAsText];
			};
