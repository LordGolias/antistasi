...
private _fnc_spawn = {
	...
	[_crate1] call AS_fnc_emptyCrate;
	[_crate2] call AS_fnc_emptyCrate;
	[_crate3] call AS_fnc_emptyCrate;
	[_crate4] call AS_fnc_emptyCrate;
...
private _fnc_wait_to_deliver = {
	...
	if (_truck distance _position < 40 and (speed _truck == 0)) then {
		...
		{
			if (isPlayer _x) then {
				[_truck,true] remoteExec ["AS_fnc_lockVehicle",_x];
			};
