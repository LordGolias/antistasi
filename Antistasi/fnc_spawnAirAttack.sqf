...
if (_airType in ["planes","armedHelis"]) then {
	...
	[_veh,"Air Attack"] spawn AS_fnc_setConvoyImmune;
} else {
	...
	if (_seats <= 15) then {
		...
		[_grupoVeh, _posorigen, _landpos, _location, _grupo, 25*60, "air"] call AS_QRF_fnc_dismountTroops;
		...
		[_veh,"Air Transport"] spawn AS_fnc_setConvoyImmune;
		...
		{[_x] spawn AS_fnc_initUnitAAF;_x assignAsCargo _veh;_x moveInCargo _veh; _soldiers = _soldiers + [_x]} forEach units _grupo;
		_groups pushBack _grupo;
		_grupo1 = [_posorigen, side_red, _tipogrupo] call BIS_Fnc_spawnGroup;
		{[_x] spawn AS_fnc_initUnitAAF;_x assignAsCargo _veh;_x moveInCargo _veh; _soldiers = _soldiers + [_x]} forEach units _grupo1;
		_groups pushBack _grupo;
		[_grupoVeh, _pos, _posdestino, _location, [_grupo, _grupo1], 25*60] call AS_QRF_fnc_fastrope;
	};
};
