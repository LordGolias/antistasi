...
Devin = _groupDev createUnit ["C_Nikos", [8173.79,25308.9,0.00156975], [], 0.9, "NONE"];
...
if (random 8 < 1) then {
	...
	{_x assignAsCargo _veh; _x moveInCargo _veh; _soldados = _soldados + [_x]; [_x] spawn AS_fnc_initUnitOPFOR} forEach units _grupo;
...
if ({((side _x isEqualTo side_blue) || (side _x isEqualTo civilian)) && (_x distance Devin < 10)} count allPlayers > 0) then {
	...
	[[Devin,"buy_exp"],"flagaction"] call BIS_fnc_MP;
...
if (alive Devin) then {
	...
	[[Devin,"remove"],"flagaction"] call BIS_fnc_MP;
};
