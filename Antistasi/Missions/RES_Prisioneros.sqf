...
_tsk = ["RES",[side_blue,civilian],[format [_tskDesc,_nombredest,numberToDate [2035,_fechalimnum] select 3,numberToDate [2035,_fechalimnum] select 4],
	_tskTitle,_location],_posicion,"CREATED",5,true,true,"run"] call BIS_fnc_setTask;
...
waitUntil {sleep 1; ({alive _x} count _POWs == 0) or ({(alive _x) and (_x distance getMarkerPos "FIA_HQ" < 50)} count _POWs > 0) or (dateToNumber date > _fechalimnum)};
...
if ({alive _x} count _POWs == 0) then
	{
	_tsk = ["RES",[side_blue,civilian],[format [_tskDesc,_nombredest,numberToDate [2035,_fechalimnum] select 3,numberToDate [2035,_fechalimnum] select 4],
		_tskTitle,_location],_posicion,"FAILED",5,true,true,"run"] call BIS_fnc_setTask;
...
if ({(alive _x) and (_x distance getMarkerPos "FIA_HQ" < 50)} count _POWs > 0) then
	{
	_tsk = ["RES",[side_blue,civilian],[format [_tskDesc,_nombredest,numberToDate [2035,_fechalimnum] select 3,numberToDate [2035,_fechalimnum] select 4],
		_tskTitle,_location],_posicion,"SUCCEEDED",5,true,true,"run"] call BIS_fnc_setTask;
