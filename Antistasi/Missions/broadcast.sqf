...
private _fnc_spawn = {
	...
	propTruck setVariable ["destino", [_location] call localizar, true];
...
private _fnc_run = {
	...
	private _fnc_increaseCounterCondition = {
		{_x call AS_fnc_isUnconscious} count ([300, propTruck, "BLUFORSpawn"] call AS_fnc_unitsAtDistance) !=
	...
	*/
	...
	{
		if (isPlayer _x) then {
			[petros,"globalChat", _info] remoteExec ["commsMP",_x]
		}
	} forEach ([300, propTruck, "BLUFORSpawn"] call AS_fnc_unitsAtDistance);
