...
private _validLocations = [["camp", "fia_hq"], "FIA"] call AS_fnc_location_TS;
...
_origin = [_validLocations, position player] call BIS_Fnc_nearestPosition;
...
if (count _posicionTel > 0) then
	...
	if (_side != "FIA") exitWith {hint "You cannot Fast Travel to an enemy controlled zone"; openMap [false,false]};

	{
		if (((side _x == side_red) or (side _x == side_green)) and (_x distance _position < 500) and (not(captive _x))) then {_chequeo = true};
	} forEach allUnits;
	...
	if (_posicionTel distance _position < 50) then {
		...
		if ((_type == "camp") && (random 10 < 1) && !(captive player)) then {
			[_location] remoteExec ["DEF_Camp",HCattack];
			[format ["Camp under attack: %1", _location]] remoteExec ["fnc_logOutput", 2];
		};
